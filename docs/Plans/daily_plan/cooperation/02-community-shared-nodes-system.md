# [基本完成] 共享节点系统（社区提示词节点）— 详细规划

> 最后更新：2026-03-27
> 编码要求：UTF-8 (无BOM) + LF
> 状态：**[基本完成] 一期主链路已落地，剩余项待补齐**
...
## 26. 当前未完成项与后续补齐计划（超详细）

> 本节用于明确：**哪些内容已经完成，哪些内容还没有完成，为什么还没做，以及下一阶段该如何落地。**
>
> 当前状态不是“未开始”，也不是“全部完成”，而是：
>
> - **一期主链路已完成**
> - **产品化补齐项仍然存在**
> - **后台治理能力和长期运营能力尚未落地**
...
- 社区节点数据库主表与点赞表已建立
- Supabase 云端迁移已实际执行
- 公开列表、详情、发布、更新、删除、点赞、取消点赞 API 已落地
- 共享节点商店双视图已接入 NodeStorePanel
- 发布弹窗、知识文件上传、JSON Schema 编辑器、AI 生成 Schema 已落地
- 共享节点可拖入画布
- 后端 `CommunityNode` 执行类已可从数据库读取封装 prompt 并执行
- 知识文件文本已可注入 prompt
- `markdown | json` 输出约束已落地
- 执行器已修复 `node.data.config` 与根层运行字段断层问题
- 节点模型选择已真实生效，而不是停留在 UI 层
...
- 完整的“我的共享节点”前端管理界面
- 浏览器级完整冒烟测试与 E2E
- Storage 上传与权限策略的真实用户操作验证
- 发布者可控 `config_schema` 参数化能力
- 社区节点使用分析、排行榜、运营治理能力

---

### 26.2 未完成项一：管理员审核流与后台治理能力
...
这是一个明显的二期/运营期能力，因为它不影响“一期是否能形成主链路闭环”。一期的核心目标是先把：
...
如果长期维持“发布即公开”，会出现以下隐患：

1. 用户可能发布低质量 prompt，污染节点市场
2. 用户可能发布违规内容、广告内容、无意义节点
3. 一旦节点数量增长，前台列表会迅速失去信噪比
4. 后续要补审核流时，需要处理已经公开存量节点的状态迁移

#### 后续建议落地方案

##### 后端需要新增

- `GET /api/admin/community-nodes`
  - 支持按 `pending | approved | rejected` 筛选
- `POST /api/admin/community-nodes/{id}/approve`
- `POST /api/admin/community-nodes/{id}/reject`
  - 需要 `reject_reason`
- 可选：`POST /api/admin/community-nodes/{id}/hide`
  - 用于已公开节点的临时下架
...
保留现有字段即可，不需要重建表。只需要把当前“一发布即 approved”的逻辑切换为：
...
- 普通用户发布后，节点不会立即出现在公开列表
- 管理员审核通过后，节点才进入市场
- 驳回节点可在“我的节点”里看到驳回理由

---

### 26.3 未完成项二：`install_count` / 使用量排行榜能力

#### 当前现状

当前 `ss_community_nodes` 中已经有：

- `install_count INTEGER NOT NULL DEFAULT 0`

但现阶段没有以下任何真实配套：

- `ss_community_node_installs` 表
- 首次拖入画布计数
- 首次执行计数
- 去重策略
- install_count 的触发器或业务更新逻辑

换句话说，当前 `install_count` 只是一个保留字段，数值长期停留在 `0`。

#### 为什么没做

一期阶段优先级更高的是：

- 先保证“能发布、能被看到、能拖入、能执行”

而排行榜和热度分析属于“运营增强能力”，可以在主链路稳定后补。

#### 风险

如果继续保留字段但不统计，会带来两个问题：

1. 前端展示 install_count 时缺乏可信度
2. 后续若要做“最受欢迎”“最多使用”排序，将没有数据基础

#### 后续建议落地方案

##### 方案 A：以“首次拖入画布”作为 install

定义：

- 当用户第一次把某个 `community_node_id` 拖入自己的任意工作流时，记为一次 install
...
##### 方案 B：以“首次执行成功”作为 install

定义：

- 当用户的工作流中包含该社区节点，并且实际执行成功一次，记为 install
...
- 统计链路更复杂
- 要依赖 workflow run / node run 数据

##### 推荐方案

二期先做 **方案 A**，因为它和现有画布交互最容易对接。
...
- 前端拖入画布成功后
- 或后端保存工作流时检测新增的 `community_node_id`

##### Done When

- 同一用户对同一节点只计一次 install
- install_count 可被前端稳定展示
- 排序支持 `likes | newest | installs`

---

### 26.4 未完成项三：完整的“我的共享节点”前端管理页
...
- `PUT /api/community-nodes/{id}`
- `DELETE /api/community-nodes/{id}`

但前端目前主要只完成了：

- 在共享节点列表底部提供“发布我的节点”入口
- 发布成功后将新节点临时插入当前列表
...
一期优先目标是“让社区节点能被发布和被使用”，而不是先做完整创作者中心。
...
1. 用户发布后难以回头管理自己的节点
2. 后续补审核流后，作者没有地方查看状态与驳回原因
3. `update / delete` API 虽然存在，但前端没有承接入口
...
- 作者可编辑未驳回节点
- 作者可删除自己的节点
- 若节点被驳回，可清晰看到 `reject_reason`

---

### 26.5 未完成项四：Storage 上传链路与真实权限策略的端到端验证
...
但当前还没有完成真正的“浏览器真实用户上传”验证，也没有做完整的异常场景冒烟：

- 文件类型不支持
- 文件超过 10MB
- storage 上传成功但 parse 失败
- parse 成功但 storage 上传失败
- 未登录用户发布
- 不同用户读取他人文件
...
Storage 是这套系统中最容易出现“代码看着对，但真实运行有细节差异”的地方，例如：

- SDK 参数命名与预期不一致
- Storage policy 条件过严或过松
- 用户文件夹命名和 `auth.uid()` 的匹配细节偏差
- 文本提取库对某些 docx/pdf 的行为不稳定
...
本轮优先解决的是“架构与主链路打通”，不是测试平台建设。

#### 风险

如果没有浏览器级验证，就还存在以下未知数：

- 共享节点卡片拖拽在不同浏览器中的兼容性
- 发布弹窗的真实交互细节
- 画布节点渲染与执行输出联动
- 点赞按钮的 optimistic update 行为
- 登录态失效时的体验
...
优先做 3 条 Playwright case：

1. `publish-community-node.spec.ts`
2. `like-and-add-community-node.spec.ts`
3. `execute-community-node-workflow.spec.ts`
...
### 26.7 未完成项六：发布者可控 `config_schema` 参数化能力
...
然后让使用者在画布节点配置中填写这些参数，而不是只能改模型和 label。

但是当前一期实际上没有开放这个能力，原因是：

- 一旦开放，就意味着前后端都要支持“用户自定义参数模板”
- 这会显著增加安全边界与 prompt 模板拼接复杂度
...
这是从“共享 prompt 节点”走向“可复用参数化节点”的关键台阶，但复杂度明显高于一期。
...
格式参考现有官方节点的 `config_schema`

##### 前端

- 发布表单新增“高级参数”编辑区
- 画布节点配置抽屉读取社区节点的 `config_schema`
- 使用者填写参数后写入 `node.data.config`

##### 后端

- `CommunityNode.execute()` 把 `node.data.config` 拼入 user message 或 prompt 模板
- 对参数做白名单校验

##### 安全要求

- 不允许发布任意脚本型参数
- 仅允许标量、枚举、布尔、有限 JSON 对象
...
### 26.8 未完成项七：市场化运营能力与内容治理补充

#### 当前现状

当前商店已经具备“看、搜、赞、发、拖入”的基础能力，但还没有形成真正的市场化产品能力，例如：
...
- 这三项最直接影响“用户能不能顺畅使用和维护节点”
- 改动范围相对可控

### P2：再补运营基础能力

1. install_count 真实统计
2. 排序支持 installs
3. 精选 / 推荐逻辑
...
- **不是纯草案**
- **不是 0 到 1 的待设计文档**
- **而是“一期主链路已基本完成 + 剩余项待继续补齐”的实施文档**

因此，后续继续推进时，不应再从“是否要做社区节点”开始讨论，而应直接进入：
