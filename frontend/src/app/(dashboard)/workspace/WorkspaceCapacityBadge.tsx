...
export default function WorkspaceCapacityBadge({ quota }: WorkspaceCapacityBadgeProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* ── Popover Panel (Desktop) / Sheet Panel (Mobile) ── */}
      {open && !isMobile && (
        <div
          className="absolute right-0 top-full mt-2 z-50 w-[320px] rounded-2xl
                     ...
                     overflow-hidden"
        >
          <CapacityPanelContent quota={quota} isUltra={isUltra} ratio={ratio} progressPct={progressPct} tierInfo={tierInfo} onClose={() => setOpen(false)} />
        </div>
      )}

      {/* Mobile Sheet */}
      {open && isMobile && (
        <MobileCapacitySheet quota={quota} isUltra={isUltra} ratio={ratio} progressPct={progressPct} tierInfo={tierInfo} onClose={() => setOpen(false)} />
      )}
    </div>
  );
}

// Extracted panel content component for reuse
function CapacityPanelContent({
  quota,
  isUltra,
  ratio,
  progressPct,
  ...
  ratio: number;
  progressPct: number;
  tierInfo: { label: string; icon: React.ReactNode; color: string };
  onClose: () => void;
}) {
  return (
    <>
      ...
      <div className="relative bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/50 px-5 py-4">
        ...
        <div className="relative flex items-start justify-between gap-3">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors mt-0.5">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
      ...
      <div className="px-5 py-4 flex flex-col gap-4">
        ...
        {/* Tier comparison */}
        {!isUltra && <TierComparison currentTier={quota.tier} />}
        ...
        {/* CTA section */}
        {!isUltra && <UpgradeCTA currentTier={quota.tier} />}
      </div>
    </>
  );
}
...
function UpgradeCTA({ currentTier }: { currentTier: string }) {
  const upgradeTier = currentTier === 'free' ? 'Pro' : currentTier === 'pro' ? 'Pro+' : null;
  return (
...
function MobileCapacitySheet({
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      {/* Sheet */}
      <div className="absolute bottom-0 left-0 right-0 bg-background rounded-t-2xl shadow-2xl max-h-[80vh] overflow-hidden flex flex-col">
        {/* Drag Handle */}
        ...
        <div className="flex items-center justify-between px-4 pb-3 border-b border-border">
          <span className="font-serif text-lg font-semibold">工作流容量</span>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto p-4 flex flex-col gap-4">
          {/* Tier badge */}
          <div className="flex items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium ${tierInfo.color}`}>
              {tierInfo.icon}
              {tierInfo.label}
            </span>
            <span className="text-sm text-muted-foreground">{getStatusMessage(ratio, isUltra)}</span>
          </div>
            ...
            <div className="bg-muted/50 rounded-xl p-4">
              <div className="flex justify-between items-baseline mb-3">
                <span className="text-sm text-muted-foreground">
                  已使用 <strong className="text-foreground">{quota.workflows_used}</strong> 个
                  {quota.workflows_addon_qty > 0 && (
                    <span className="ml-1 text-emerald-600 dark:text-emerald-400 text-xs">(含 +{quota.workflows_addon_qty} 增值包)</span>
                  )}
                </span>
                <span className="text-xs text-muted-foreground">共 {quota.workflows_total} 个</span>
              </div>
          ...
          {isUltra && (
            <div className="text-center py-6 bg-muted/50 rounded-xl">
              ...
              <p className="text-base font-medium">Ultra 版不限工作流数量</p>
              <p className="text-sm text-muted-foreground mt-1">尽情创建，空间永不受限</p>
            </div>
          )}
          ...
          {!isUltra && (
            <div className="pt-2">
              <UpgradeCTA currentTier={quota.tier} />
            </div>
          )}
        </div>
