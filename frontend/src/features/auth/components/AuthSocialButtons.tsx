...
/**
 * Social login buttons for GitHub and Google.
 *
 * Uses Supabase's built-in OAuth flow:
 *   1. signInWithOAuth redirects to provider
 *   2. Provider redirects back to Supabase callback
 *   3. Supabase exchanges code and redirects to /auth/callback
 *   4. AuthSessionBridge picks up the SIGNED_IN event and syncs cookies
 */
export function AuthSocialButtons() {
  ...
  async function handleOAuthLogin(provider: OAuthProvider) {
    ...
    try {
      ...
      if (oauthError) {
        setError(`${providerLabel(provider)}登录失败，请重试`);
        setLoadingProvider(null);
      }
      // If no error, the browser will redirect — no need to reset state
    } catch {
