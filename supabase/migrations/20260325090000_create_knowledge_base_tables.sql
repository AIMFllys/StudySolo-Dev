...
CREATE POLICY ss_kb_document_summaries_select ON public.ss_kb_document_summaries
  FOR SELECT USING (document_id IN (SELECT id FROM public.ss_kb_documents WHERE user_id = (SELECT auth.uid())));
CREATE POLICY ss_kb_document_summaries_insert ON public.ss_kb_document_summaries
  FOR INSERT WITH CHECK (document_id IN (SELECT id FROM public.ss_kb_documents WHERE user_id = (SELECT auth.uid())));
CREATE POLICY ss_kb_document_summaries_delete ON public.ss_kb_document_summaries
  FOR DELETE USING (document_id IN (SELECT id FROM public.ss_kb_documents WHERE user_id = (SELECT auth.uid())));
